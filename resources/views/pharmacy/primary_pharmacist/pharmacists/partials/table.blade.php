@if($pharmacists->count() > 0)
    @foreach($pharmacists as $pharmacist)
        <tr class="pharmacist-row" data-name="{{ strtolower($pharmacist->name) }}" data-email="{{ strtolower($pharmacist->email) }}">
            ...
            <td><a href="{{ route('primary_pharmacist.pharmacists.show', $pharmacist) }}">{{ $pharmacist->name }}</a></td>
            ...
            <td>
                <div class="btn-group">
                    ...
                    <div class="dropdown-menu">
                        <a class="dropdown-item" href="{{ route('primary_pharmacist.pharmacists.show', $pharmacist) }}">
                ...
                <!-- Activate Form -->
                <form id="activate-form-{{ $pharmacist->id }}" action="{{ route('primary_pharmacist.pharmacists.update', $pharmacist) }}" method="POST" style="display: none;">
                    @csrf
                    @method('PUT')
                ...
                <!-- Suspend Form -->
                <form id="suspend-form-{{ $pharmacist->id }}" action="{{ route('primary_pharmacist.pharmacists.update', $pharmacist) }}" method="POST" style="display: none;">
                    @csrf
                    @method('PUT')
                ...
                <!-- Delete Form -->
                <form id="delete-form-{{ $pharmacist->id }}" action="{{ route('primary_pharmacist.pharmacists.destroy', $pharmacist) }}" method="POST" style="display: none;">
                    @csrf
                    @method('DELETE')
                </form>
            </td>
