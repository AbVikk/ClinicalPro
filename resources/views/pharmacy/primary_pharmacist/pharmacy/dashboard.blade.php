        ...
        <div class="row">
            ...
            <div class="col-lg-7 col-md-7 col-sm-12 text-right">
                <ul class="breadcrumb float-md-right">
                    <li class="breadcrumb-item"><a href="{{ route('primary_pharmacist.dashboard') }}"><i class="zmdi zmdi-home"></i> Primary Pharmacist</a></li>
    ...
    <div class="container-fluid">
        ...
        <div class="row clearfix">
            <div class="col-lg-12 col-md-12">
                <div class="card" id="pharmacy-overview">
                    <div class="header">
                        ...
                        <ul class="header-dropdown">
                            <li class="dropdown"> <a href="javascript:void(0);" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"> <i class="zmdi zmdi-more"></i> </a>
                                <ul class="dropdown-menu dropdown-menu-right slideUp float-right">
                                    <li><a href="{{ route('primary_pharmacist.pharmacy.drugs.create.form') }}">Add New Drug</a></li>
                                    <li><a href="{{ route('primary_pharmacist.pharmacy.drugs.all') }}">View All Drugs</a></li>
                                    <li><a href="{{ route('primary_pharmacist.pharmacy.categories.index') }}">Manage Categories</a></li>
                                    <li><a href="{{ route('primary_pharmacist.pharmacy.mg.index') }}">Manage MG Values</a></li>
                                    <li><a href="{{ route('primary_pharmacist.pharmacy.stock.receive') }}">Receive Stock</a></li>
                                </ul>
                            </li>
                        </ul>
                    </div>                    
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-right">
                                                            <a class="dropdown-item" href="{{ route('primary_pharmacist.pharmacy.drugs.view', $drug->id) }}">View Details</a>
                                                            <a class="dropdown-item" href="{{ route('primary_pharmacist.pharmacy.drugs.edit', $drug->id) }}">Edit Medicine</a>
                            ...
                            <div class="tab-pane" id="prescription">
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            @foreach(\App\Models\Drug::where('is_prescription', true)->with('batches')->limit(10)->get() as $drug)
                                            <tr>
                                                <td>{{ $drug->id }}</td>
                                                <td>{{ $drug->name }}</td>
                                                <td>{{ $drug->category }}</td>
                            ...
                            <div class="tab-pane" id="otc">
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            @foreach(\App\Models\Drug::where('is_prescription', false)->with('batches')->limit(10)->get() as $drug)
                                            <tr>
                                                <td>{{ $drug->id }}</td>
                                                <td>{{ $drug->name }}</td>
                                                <td>{{ $drug->category }}</td>
...
<script>
    ...
    $('#update-stock-btn').on('click', function() {
        ...
        $.ajax({
            url: '{{ route('primary_pharmacist.pharmacy.stock.update') }}',
            method: 'POST',
            data: formData,
    ...
    $('#viewHistoryModal').on('show.bs.modal', function (event) {
        ...
        $.ajax({
            url: '/primary_pharmacist/pharmacy/drugs/' + drugId + '/history',
            method: 'GET',
            success: function(response) {
    ...
    $('#confirm-delete-btn').on('click', function() {
        if (deleteDrugId) {
            ...
            var form = $('<form>', {
                'method': 'POST',
                'action': '/primary_pharmacist/pharmacy/drugs/' + deleteDrugId
            });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function filterMedicines() {
        ...
        // Filter medicines in all tabs
        const allTabRows = document.querySelectorAll('#all tbody tr');
        const prescriptionTabRows = document.querySelectorAll('#prescription tbody tr');
        const otcTabRows = document.querySelectorAll('#otc tbody tr');
        const controlledTabRows = document.querySelectorAll('#controlled tbody tr');
        
        const allRows = [...allTabRows, ...prescriptionTabRows, ...otcTabRows, ...controlledTabRows];
    ...
    function initializeDropdowns() {
        // Ensure all dropdown toggles have the proper attributes
        ...
        $('.dropdown-toggle').off('click.dropdownFallback').on('click.dropdownFallback', function(e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        $('.dropdown-toggle').off('show.bs.dropdown').on('show.bs.dropdown', function () {
            console.log('Dropdown shown:', this);
            $(this).attr('aria-expanded', 'true');
        });
        
        $('.dropdown-toggle').off('hide.bs.dropdown').on('hide.bs.dropdown', function () {
    ...
    // Also initialize dropdowns when tabs are switched
    $('a[data-toggle="tab"]').off('shown.bs.tab').on('shown.bs.tab', function (e) {
        setTimeout(initializeDropdowns, 50);
    });
    
    // Force re-initialization after a longer delay to ensure all elements are loaded
    setTimeout(initializeDropdowns, 1000);
    
    // Re-initialize dropdowns when the window is resized
