...
<title>:: Clinical Pro :: All Drugs</title>
<!-- Favicon -->
    ...
    <div class="block-header">
        <div class="row">
            <div class="col-lg-5 col-md-5 col-sm-12">
                <h2>All Drugs
                <small>Complete list of all medicines in the system</small>
                </h2>
            </div>            
            <div class="col-lg-7 col-md-7 col-sm-12 text-right">
                <ul class="breadcrumb float-md-right">
                    ...
                    <li class="breadcrumb-item active">All Drugs</li>
                </ul>
            </div>
        </div>
    </div>
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $drug->name }}</td>
                                        <td>{{ $drug->category }}</td>
                                        {{-- 1. TOTAL RECEIVED (History - Never decreases) --}}
                                        <td>
                                            <span class="text-muted">{{ $drug->batches->sum('received_quantity') }}</span>
                                        </td>

                                        {{-- 2. LAST RECEIVED (Most recent batch) --}}
                                        <td>
                                            @php
                                                $lastBatch = $drug->batches->sortByDesc('created_at')->first();
                                            @endphp
                                            @if($lastBatch)
                                                <strong>{{ $lastBatch->received_quantity }}</strong>
                                                <small class="d-block text-muted">{{ $lastBatch->created_at->format('M d') }}</small>
                                            @else
                                                -
                                            @endif
                                        </td>

                                        {{-- 3. CURRENT STOCK (Live Inventory) --}}
                                        <td>
                                            @php
                                                $currentStock = $drug->clinicInventories->sum('stock_level');
                                                $stockClass = $currentStock == 0 ? 'text-danger' : 'text-success';
                                            @endphp
                                            <strong class="{{ $stockClass }}">{{ $currentStock }}</strong>
                                        </td>

                                        <td>
                                            @if($drug->batches->isNotEmpty())
                                                {{ $drug->batches->sortBy('expiry_date')->first()->expiry_date->format('Y-m-d') }}
                                            @else
                                                N/A
                                            @endif
                                        </td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const medicineRows = document.querySelectorAll('tbody tr');
